...
class ANETdetection(object):
    ...
    def __init__(self, ground_truth_json=None, prediction_json=None,
                 class_labels=None,
        ...
        self.ground_truth, self.activity_index = self._import_ground_truth(
            ground_truth_json, class_labels)
        self.prediction = self._import_prediction(prediction_json)
    ...
    def _import_ground_truth(self, ground_truth_json, class_labels=None):
        ...
        if class_labels is not None:
            for class_label in class_labels:
                activity_index[class_label] = cidx
                cidx += 1
    ...
    def _import_prediction(self, prediction_json):
    ...
    def _get_ground_truths_with_label(self, ground_truth_by_label, label_name, cidx):
        ...
        try:
            ...
            print('Warning: No GT of label \'%s\' were provdied.' % label_name)
            return pd.DataFrame()
    ...
    def wrapper_compute_average_precision(self):
        ...
        results = Parallel(n_jobs=len(self.activity_index))(
            delayed(compute_average_precision_detection)(
                ground_truth=self._get_ground_truths_with_label(ground_truth_by_label, label_name, cidx),
                prediction=self._get_predictions_with_label(prediction_by_label, label_name, cidx),
                tiou_thresholds=self.tiou_thresholds,
            ) for label_name, cidx in self.activity_index.items())
    ...
    def evaluate(self):
        ...
        self.interpolated_ap = self.ap
        self.interpolated_mAP = self.mAP

        if self.verbose:
            print('[RESULTS] Performance on ActivityNet detection task.')
            print('\tAverage-mAP: {}'.format(self.average_mAP))

        return self.interpolated_mAP
